#include "stdio.h"
//fix "_0_" --> "_1_"
#include "conv_13_weight_bn_short_pad.h"
...
int main()
{
  FILE *fp = fopen("group_13_weight_it.h","w");//fix    " " or "_it"
  int i,j,k; //cnt
  fprintf(fp,"short group_13_biases[]={");
  for(i=0;i<sizeof(group_13_biases)/sizeof(short);i++)
    {
      fprintf(fp,"%hd,\n",group_13_biases[i]);
    }
  fseek(fp,-2,SEEK_CUR);
  ...
  for(i=0;i<input_fold_factor;i++)
      ...
      for(j=0;j<output_ch;j++)
	{
	  for(k=0;k<N_max*12;k++)
	    {
	      fprintf(fp,"%hd,\n",group_13_weights[j*12*N_max*input_fold_factor+12*N_max*i+k]);
