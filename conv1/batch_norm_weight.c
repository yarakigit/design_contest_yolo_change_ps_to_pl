...
void main()
{
    ...
    fp = fopen(path,"w");
    int output_channel = sizeof(conv_1_rolling_mean)/sizeof(float);
    int weight_size = sizeof(conv_1_weights)/sizeof(float);
    ...
    {
        float bias = conv_1_biases[i];
        float scale = conv_1_scales[i];
        float mean = conv_1_rolling_mean[i];
        float var = conv_1_rolling_variance[i];
	...
	int j;
        ...
        {
            float weight = conv_1_weights[i*kernel_size+j];
