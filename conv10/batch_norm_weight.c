...
void main()
{
    ...
    fp = fopen(path,"w");
    int output_channel = sizeof(conv_10_rolling_mean)/sizeof(float);
    int weight_size = sizeof(conv_10_weights)/sizeof(float);
    ...
    {
        float bias = conv_10_biases[i];
        float scale = conv_10_scales[i];
        float mean = conv_10_rolling_mean[i];
        float var = conv_10_rolling_variance[i];
	...
	int j;
        ...
        {
            float weight = conv_10_weights[i*kernel_size+j];
