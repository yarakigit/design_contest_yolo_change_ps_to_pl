...
void main()
{
    ...
    fp = fopen(path,"w");
    int output_channel = sizeof(conv_11_rolling_mean)/sizeof(float);
    int weight_size = sizeof(conv_11_weights)/sizeof(float);
    ...
    {
        float bias = conv_11_biases[i];
        float scale = conv_11_scales[i];
        float mean = conv_11_rolling_mean[i];
        float var = conv_11_rolling_variance[i];
	...
	int j;
        ...
        {
            float weight = conv_11_weights[i*kernel_size+j];
