...
void main()
{
    ...
    fp = fopen(path,"w");
    int output_channel = sizeof(conv_5_rolling_mean)/sizeof(float);
    int weight_size = sizeof(conv_5_weights)/sizeof(float);
    ...
    {
        float bias = conv_5_biases[i];
        float scale = conv_5_scales[i];
        float mean = conv_5_rolling_mean[i];
        float var = conv_5_rolling_variance[i];
	...
	int j;
        ...
        {
            float weight = conv_5_weights[i*kernel_size+j];
