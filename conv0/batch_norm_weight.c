...
void main()
{
  ...
  fp = fopen(path,"w");
  int output_channel = sizeof(conv_0_rolling_mean)/sizeof(float);
  int weight_size = sizeof(conv_0_weights)/sizeof(float);
  ...
  {
    float bias = conv_0_biases[i];
    float scale = conv_0_scales[i];
    float mean = conv_0_rolling_mean[i];
    float var = conv_0_rolling_variance[i];
  ...
  for(i=0; i<output_channel; i++)
  {
    ...
    {
      float weight = conv_0_weights[i*kernel_size+j];
