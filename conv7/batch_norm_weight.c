...
void main()
{
    ...
    fp = fopen(path,"w");
    int output_channel = sizeof(conv_7_rolling_mean)/sizeof(float);
    int weight_size = sizeof(conv_7_weights)/sizeof(float);
    ...
    {
        float bias = conv_7_biases[i];
        float scale = conv_7_scales[i];
        float mean = conv_7_rolling_mean[i];
        float var = conv_7_rolling_variance[i];
	...
	int j;
        ...
        {
            float weight = conv_7_weights[i*kernel_size+j];
