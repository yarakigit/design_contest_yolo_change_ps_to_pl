...
void main()
{
    ...
    fp = fopen(path,"w");
    int output_channel = sizeof(conv_8_rolling_mean)/sizeof(float);
    int weight_size = sizeof(conv_8_weights)/sizeof(float);
    ...
    {
        float bias = conv_8_biases[i];
        float scale = conv_8_scales[i];
        float mean = conv_8_rolling_mean[i];
        float var = conv_8_rolling_variance[i];
	...
	int j;
        ...
        {
            float weight = conv_8_weights[i*kernel_size+j];
