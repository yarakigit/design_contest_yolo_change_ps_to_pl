...
void main()
{
    ...
    fp = fopen(path,"w");
    int output_channel = sizeof(conv_4_rolling_mean)/sizeof(float);
    int weight_size = sizeof(conv_4_weights)/sizeof(float);
    ...
    {
        float bias = conv_4_biases[i];
        float scale = conv_4_scales[i];
        float mean = conv_4_rolling_mean[i];
        float var = conv_4_rolling_variance[i];
	...
	int j;
        ...
        {
            float weight = conv_4_weights[i*kernel_size+j];
