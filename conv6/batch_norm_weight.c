...
void main()
{
    ...
    fp = fopen(path,"w");
    int output_channel = sizeof(conv_6_rolling_mean)/sizeof(float);
    int weight_size = sizeof(conv_6_weights)/sizeof(float);
    ...
    {
        float bias = conv_6_biases[i];
        float scale = conv_6_scales[i];
        float mean = conv_6_rolling_mean[i];
        float var = conv_6_rolling_variance[i];
	...
	int j;
        ...
        {
            float weight = conv_6_weights[i*kernel_size+j];
