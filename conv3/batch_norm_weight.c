...
void main()
{
    ...
    fp = fopen(path,"w");
    int output_channel = sizeof(conv_3_rolling_mean)/sizeof(float);
    int weight_size = sizeof(conv_3_weights)/sizeof(float);
    ...
    {
        float bias = conv_3_biases[i];
        float scale = conv_3_scales[i];
        float mean = conv_3_rolling_mean[i];
        float var = conv_3_rolling_variance[i];
	...
	int j;
        ...
        {
            float weight = conv_3_weights[i*kernel_size+j];
